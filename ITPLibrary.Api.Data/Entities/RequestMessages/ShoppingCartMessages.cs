namespace ITPLibrary.Api.Data.Entities.RequestMessages
{
    public static class ShoppingCartMessages
    {
        public const string Success = "Success";
        public const string FailedToAddToCart = "Something went wrong, your book could not be added to the shopping cart";
        public const string FailedToDeleteFromCart = "Something went wrong, your book could not be deleted from the shopping cart";
        public const string FailedToGetCart= "Something went wrong, your shopping cart could not be retrieved";
    }
}
